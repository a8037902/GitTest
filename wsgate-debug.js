...
wsgate.RDP = new Class( {
    ...
    Disconnect: function() {
		//20190822 xuhan
		window.removeEvent('resize', this.OnDesktopSize);
        this._reset();
    },
    ...
    SetupCanvas: function(canvas){
		...
		//20190822 xuhan
		window.addEvent('resize', this.OnDesktopSize);
	...
	//20190822 xuhan
	OnDesktopSize: function () {
		clearTimeout(rt);
		rt = setTimeout(function(){
			if(rdp.open){
				
				rdp.removeEvent('disconnected',OnDisconnected);
				rdp.Disconnect();
		...
		//this.Disconnect();
               //this.ResizeCanvas($('dtsize').value);
            },
    /**
            ...
            case 2:
                ...
                if ((bpp == 16) || (bpp == 15)) {
					...
					if (this._ckclp(x, y) && this._ckclp(x + dw, y + dh)) {
                        ...
                        var outB = this.cctx.createImageData(w, h);
                        if (compressed) {
                            wsgate.dRLE32_RGBA(bmdata, len, w, h, outB.data);
                            //wsgate.flipV(outB.data, w, h);
                        } else {
                            //wsgate.dRGB162RGBA(bmdata, len, outB.data);
                        }
...
wsgate.pel24ToRGBA = function (pel, outA, outI) {
    var pelR = (pel >> 16)& 0x000000FF;
    var pelG = (pel >> 8)&0x000000FF;
    var pelB = pel & 0x000000FF;
...
wsgate.xorbufRGBAPel24 = function(inA, inI, outA, outI, pel) {
    var pelR = (pel >> 16)& 0x000000FF;
    var pelG = (pel >> 8)&0x000000FF;
    var pelB = pel & 0x000000FF;
...
wsgate.WriteFirstLineFgBgImage24toRGBA = function(outA, outI, bitmask, fgPel, cBits) {
    var cmpMask = 0x01;

    while (cBits-- > 0) {
        if (bitmask & cmpMask) {
            wsgate.pel24ToRGBA(fgPel, outA, outI);
        } else {
            wsgate.pel24ToRGBA(0, outA, outI);
        }
        outI += 4;
        cmpMask <<= 1;
    }
...
wsgate.WriteFgBgImage24toRGBA = function(outA, outI, rowDelta, bitmask, fgPel, cBits) {
    var cmpMask = 0x01;

    while (cBits-- > 0) {
        if (bitmask & cmpMask) {
            wsgate.xorbufRGBAPel24(outA, outI - rowDelta, outA, outI, fgPel);
        } else {
            wsgate.copyRGBA(outA, outI - rowDelta, outA, outI);
        }
        outI += 4;
...
wsgate.dRLE24_RGBA = function(inA, inLength, width, outA) {
    var runLength;
    var code, pixelA, pixelB, bitmask;
    var inI = 0;
    var outI = 0;
    ...
    while (inI < inLength) {
        ...
        switch (code) {
            ...
            case 0xF6:
                ...
                inI += advance.val;
                if (code == 0x0C || code == 0xF6) {
                    fgPel = inA[inI] | (inA[inI + 1] << 8)| (inA[inI + 2] << 16);
                    inI += 3;
                }
                if (fFirstLine) {
                    while (runLength > 0) {
            ...
            case 0xF8:
                runLength = wsgate.ExtractRunLength(code, inA, inI, advance);
                inI += advance.val;
                pixelA = inA[inI] | (inA[inI + 1] << 8) | (inA[inI + 2] << 16);
                inI += 3;
                pixelB = inA[inI] | (inA[inI + 1] << 8) | (inA[inI + 2] << 16);
                inI += 3;
                while (runLength > 0) {
                    wsgate.pel24ToRGBA(pixelA, outA, outI);
                    outI += 4;
                    wsgate.pel24ToRGBA(pixelB, outA, outI);
                    outI += 4;
                    runLength -= 1;
                }
                break;
            ...
            case 0xF7:
                ...
                if (fFirstLine) {
                    while (runLength >= 8) {
                        bitmask = inA[inI++];
                        outI = wsgate.WriteFirstLineFgBgImage24toRGBA(outA, outI, bitmask, fgPel, 8);
                        runLength -= 8;
                    }
                } else {
                    while (runLength >= 8) {
                        bitmask = inA[inI++];
                        outI = wsgate.WriteFgBgImage24toRGBA(outA, outI, rowDelta, bitmask, fgPel, 8);
                        runLength -= 8;
                    }
                }
                if (runLength > 0) {
                    bitmask = inA[inI++];
                    if (fFirstLine) {
                        outI = wsgate.WriteFirstLineFgBgImage24toRGBA(outA, outI, bitmask, fgPel, runLength);
                    } else {
                        outI = wsgate.WriteFgBgImage24toRGBA(outA, outI, rowDelta, bitmask, fgPel, runLength);
                    }
                }
                break;
            case 0x04:
            case 0xF4:
                ...
                inI += advance.val;
                while (runLength > 0) {
                    wsgate.pel24ToRGBA(inA[inI] | (inA[inI + 1] << 8) | (inA[inI + 2] << 16), outA, outI);
                    inI += 3;
                    outI += 4;
                    runLength -= 1;
                }
            ...
            case 0xF9:
                inI += 1;
                if (fFirstLine) {
                    outI = wsgate.WriteFirstLineFgBgImage24toRGBA(outA, outI, 0x03, fgPel, 8);
                } else {
                    outI = wsgate.WriteFgBgImage24toRGBA(outA, outI, rowDelta, 0x03, fgPel, 8);
                }
                break;
            case 0xFA:
                inI += 1;
                if (fFirstLine) {
                    outI = wsgate.WriteFirstLineFgBgImage24toRGBA(outA, outI, 0x05, fgPel, 8);
                } else {
                    outI = wsgate.WriteFgBgImage24toRGBA(outA, outI, rowDelta, 0x05, fgPel, 8);
                }
                break;
            case 0xFD:
                inI += 1;
                wsgate.pel24ToRGBA(0xFFFFFF, outA, outI);
                outI += 4;
                break;
            case 0xFE:
                inI += 1;
...
wsgate.dRLE32_RGBA = function(inA, inLength, width,height, outA) {
	var status;
	var FormatHeader;
	var nDstStep=width*4;
	var cs;
	var cll;
	var rle;
	var alpha;
	var planeSize;
	var rleSizes = [ 0, 0, 0, 0 ];
	var rawSizes= [ 0, 0, 0, 0 ];
	var rawWidths= [ 0, 0, 0, 0 ];
	var rawHeights= [ 0, 0, 0, 0 ];
	var planes = [ 0, 0, 0, 0 ];
	var srcp=0;
	var useAlpha=false;
	var vFlip=true;
	
	FormatHeader = inA[srcp++];
	
	cll = (FormatHeader&0x07);
	cs = (FormatHeader & (1<<3)) ? true : false;
	rle = (FormatHeader & (1<<4)) ? true : false;
	alpha = (FormatHeader & (1<<5)) ? false : true;
	...
	if(rle){
		if(alpha){
			planes[3] = srcp;
			rleSizes[3] = planar_skip_plane_rle(inA, planes[3], inLength, rawWidths[3], rawHeights[3]); /* AlphaPlane */
		...
		rleSizes[0] = wsgate.planar_skip_plane_rle(inA, planes[0], inLength, rawWidths[0], rawHeights[0]); /* RedPlane */
											
		if (rleSizes[0] < 0)
			return false;
		
		planes[1] = planes[0] + rleSizes[0];
		rleSizes[1] = wsgate.planar_skip_plane_rle(inA, planes[1], inLength, rawWidths[1], rawHeights[1]); /* GreenPlane */

		if (rleSizes[1] < 1)
			return false;

		planes[2] = planes[1] + rleSizes[1];
		rleSizes[2] = wsgate.planar_skip_plane_rle(inA, planes[2], inLength, rawWidths[2], rawHeights[2]); /* BluePlane */
		...
		if (alpha)
		{
			...
			if ((planes[2] + rawSizes[2]) > inLength)
				return false;
		}
		else
		{
			if ((inLength - srcp) < (planeSize * 3))
				return false;
	...
	{
		var pTempData = outA;
		var nTempStep = nDstStep;
		
		if(rle){
			status = wsgate.planar_decompress_plane_rle(inA,planes[0], rleSizes[0], pTempData, nTempStep, 0, 0, width, height, 0, vFlip); /* RedPlane */

			if (status < 0)
				return false;

			status = wsgate.planar_decompress_plane_rle(inA,planes[1], rleSizes[1], pTempData, nTempStep,0, 0, width, height, 1, vFlip); /* GreenPlane */

			if (status < 0)
				return false;

			status = wsgate.planar_decompress_plane_rle(inA,planes[2], rleSizes[2], pTempData, nTempStep, 0, 0, width, height, 2, vFlip); /* BluePlane */
			...
			if (useAlpha)
			{
				status = planar_decompress_plane_rle(planes[3], rleSizes[3], pTempData, nTempStep, 0, 0, width, height, 3,vFlip); /* AlphaPlane */
			}
			else
				status = wsgate.planar_set_plane(0xFF, pTempData, nTempStep, 0, 0, width, height, 3, vFlip);
			...
			if (!wsgate.planar_decompress_planes_raw(inA,planes, pTempData, 0, nTempStep, 0,
			                                  0, width, height, vFlip))
				return false;
	...
	return (inLength == srcp) ? true : false;
}

wsgate.planar_decompress_planes_raw = function(pSrcData,nSrcIndex, pDstData,
                                                DstFormat, nDstStep, nXDst,
                                                nYDst, nWidth, nHeight,
                                                vFlip)
{
	var x, y;
	var beg, end, inc;
	...
	{
		var pRGB = ((nYDst + y) * nDstStep) + (nXDst * 4);
		
		for (x = 0; x < nWidth; x++)
		{
			pDstData[pRGB++]=pSrcData[pR++];
			pDstData[pRGB++]=pSrcData[pG++];
			pDstData[pRGB++]=pSrcData[pB++];
			pDstData[pRGB++]=0xFF;//pSrcData[pA++];
		}
...
wsgate.planar_skip_plane_rle = function(pSrcData, StartIndex, SrcSize, nWidth, nHeight)
{
	var x, y;
	var controlByte;
	var pRLE = StartIndex;
	...
	for (y = 0; y < nHeight; y++)
	{
		for (x = 0; x < nWidth;)
		{
			...
			controlByte = pSrcData[pRLE++];
			nRunLength = (controlByte & 0x0000000F);
			cRawBytes = ((controlByte >> 4) & 0x0000000F);
	...
	return (pRLE - StartIndex);
}

wsgate.planar_decompress_plane_rle = function(pSrcData, StartIndex, SrcSize,
                                                pDstData, nDstStep, nXDst,
                                                nYDst, nWidth, nHeight,
                                                nChannel, vFlip)
{
	var x, y;
	...
	var controlByte;
	var currentScanline;
	var previousScanline;
	var startScanline=true;
	var srcp = StartIndex;
	...
	for (y = beg; y != end; y += inc)
	{
		var dstp = ((nYDst + y) * nDstStep) + (nXDst * 4) + nChannel;
		pixel = 0;
		currentScanline = dstp;

		for (x = 0; x < nWidth;)
		{
			controlByte = pSrcData[srcp++];

			if ((srcp - StartIndex) > SrcSize)
			{
				this.log.err('error reading input buffer');
				return -1;
			}
			...
			{
				this.log.err('too many pixels in scanline');
				return -1;
			}
			...
			if (startScanline)
			{
				...
				{
					pixel = pSrcData[srcp++];
					pDstData[dstp] = pixel&0x000000FF;
					dstp += 4;
					x++;
					cRawBytes--;
				}
			...
			else
			{
				...
				while (cRawBytes > 0)
				{
					...
					deltaValue = pDstData[previousScanline+(x * 4)] + pixel;
					pDstData[dstp] = deltaValue&0x000000FF;
					dstp += 4;
					x++;
					cRawBytes--;
				}
	...
	return (srcp - StartIndex);
}

wsgate.planar_set_plane = function(bValue, pDstData, nDstStep, nXDst,
                                     nYDst, nWidth, nHeight, nChannel,
                                     vFlip)
{
	var x, y;
	var beg, end, inc;
